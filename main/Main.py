...
def options():
    ...
    while not terminated:
        ...
        forestDensityTextRect = forestDensityText.get_rect(center=(G.SCREEN_WIDTH/2, 100))
        G.SCREEN.blit(forestDensityText, forestDensityTextRect)

        # Forest density buttons
        forestDensity_25  = Button('25%',  ((G.SCREEN_WIDTH / 2) - 400, 150), [150, 52, 25])
        forestDensity_50  = Button('50%',  ((G.SCREEN_WIDTH / 2) - 175, 150), [150, 52, 25])
        forestDensity_75  = Button('75%',  ((G.SCREEN_WIDTH / 2) + 25,  150), [150, 52, 25])
        forestDensity_100 = Button('100%', ((G.SCREEN_WIDTH / 2) + 250, 150), [150, 52, 25])
...
def automata_main():
    ...
    hexagons = C.init_hexagons(flat_top=True)
    terminated = False
    pause = True
    buttons = []
    # Simulation loop    
    while not terminated:
        ...
        if pause:
            pauseText = G.FONT.render('Simulation Paused', True, G.TEXT_COLOUR)
            pauseRect = pygame.rect.Rect((G.SCREEN_WIDTH/2 - 250, G.SCREEN_HEIGHT - 185), (500, 50))
            pauseTextRect = pauseText.get_rect(center = pauseRect.center)
            G.SCREEN.blit(pauseText, pauseTextRect)
        else: pass
        # Event handler
        for event in pygame.event.get():
            # Quit
